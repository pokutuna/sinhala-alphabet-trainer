...
> **「文字数」は数え方で 2 桁変わる(① 部品 ~80 → ② 論理上限 ≈108,500 → ④ 実在 3,052 →
> ⑤ 実用コア ~450)。** 5 段階の実数・カバレッジ・数え方の注意は
> [akshara-grammar.md §4-3](./akshara-grammar.md)。**視覚単位(`\X`)で数えるのが正しく**、
...
- **(a)=(b) の理由**: この 5,471 は規則生成集合なので生成時点で全て文法成立。renderable の「ゴミ」(描けるが文法 NG)
  も文法の取りこぼし(実在だが文法 NG)も **0 種**だった。妥当性の本当の分かれ目は **(b)文法 vs (c)実在**で、
  文法成立 5,471 − 実在 1,951 = **3,520 種が「規則上は作れるが一度も使われない」**(`ඞැ` `ඞෑ` 等、稀な子音への母音記号付け)。
